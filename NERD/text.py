...
class BaseTextClassifier:
    ...
    def query_new_example(self, mode='entropy'):
        ...
        if mode == 'entropy':
            ...
            unlab.index = range(len(unlab))
            # print(unlab[self.text_col])
            ...
            # if self.model:
            #     preds = self.model.predict(
            #         [self.all_data.loc[current_example_index][self.text_col]], as_numpy = True
            #     )
            #     if self.multilabel:
            #         preds = list(np.where(preds == 1)[1])
            #         toret['predictions'] = [int(items) for items in preds]
            #     else:
            #         preds = [preds[0]]
            #         toret['predictions'] = [int(items) for items in preds]

            print(f'Loading new example : {toret}')
            return toret
    ...
    def update_model(self):
        ...
        # self.model.fit(lab[self.text_col].tolist(), labels)
        # num_epochs = 2, batch_size = 8,
        #                learning_rate=1e-2, body_learning_rate = 2e-5,
        #                max_length=128, show_progress_bar = True
    ...
    def save_data(self, filepath=None):
        ...
        self.all_data[[self.text_col, self.label_col]].to_csv(filepath, index=False)
...
class TextClassifier:
    ...
    def start_server(self, host=None, port=8080):
        """
